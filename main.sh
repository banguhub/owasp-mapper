...
# ── Global Variables ──────────────────────────────────────────────────────────
TARGET_DOMAIN=""
TARGET_TYPE="remote"   # "local" or "remote" — set by detect_target_type()
DO_SUBDOMAIN="n"
SELECTED_CATEGORIES=""
...
# Sets global TARGET_TYPE: "local" or "remote"
detect_target_type() {
    local target="$1"
    local host="${target%%:*}"  # strip port
...
prompt_domain() {
    ...
    echo -e "  ${DIM}  • example.com          (public domain)${RESET}"
    echo -e "  ${DIM}  • localhost:8080        (local app)${RESET}"
    echo -e "  ${DIM}  • 127.0.0.1:3000        (local IP + port)${RESET}"
    echo -e "  ${DIM}  • 192.168.1.10:80       (internal network)${RESET}"
    echo ""

    while true; do
        echo -ne "${BOLD}  Enter target ${DIM}(domain / IP:port / localhost:port)${RESET}${BOLD}: ${RESET}"
        ...
        if validate_domain "$TARGET_DOMAIN"; then
            detect_target_type "$TARGET_DOMAIN"
            log_success "Target set → ${BOLD}$TARGET_DOMAIN${RESET} ${DIM}[${TARGET_TYPE}]${RESET}"

            # Warn if local target
            if [[ "$TARGET_TYPE" == "local" ]]; then
                log_info "Local/internal target detected."
                log_info "Subdomain enumeration & archive-based URL collection will be skipped."
                log_info "Using built-in crawler for URL discovery."
            fi
            break
        else
            log_error "Invalid target: '$TARGET_DOMAIN'"
            log_error "Examples: example.com | localhost:8080 | 192.168.1.10:3000"
        fi
    done
}

# ── Domain / IP / localhost Validation ───────────────────────────────────────
validate_domain() {
    local input="$1"
    [[ -z "$input" ]] && return 1
    ...
    # Standard domain (e.g. example.com, sub.example.co.uk)
...
prompt_subdomain() {
    ...
    if [[ "${TARGET_TYPE:-remote}" == "local" ]]; then
        DO_SUBDOMAIN="n"
        log_info "Local target — subdomain enumeration skipped automatically."
        return 0
    fi
